...
for /L %%i in (1, 1, %FABRIC_START_TIMEOUT%) do (
    ...
    docker exec -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org1.example.com/msp" <%= name %>_peer0.org1.example.com peer channel list > NUL 2>&1
    if errorlevel 1 (
...
docker exec -e "CORE_PEER_LOCALMSPID=Org1MSP" -e "CORE_PEER_MSPCONFIGPATH=/etc/hyperledger/msp/users/Admin@org1.example.com/msp" <%= name %>_peer0.org1.example.com peer channel getinfo -c mychannel
if errorlevel 1 (
